...
public class ERSA {
   ...
   private BigInteger modulusM;
   private BigInteger modulus;

   // generate an N-bit (roughly) public and private key
   ERSA(int N) {
      BigInteger p = BigInteger.probablePrime(N/4, random);
      BigInteger q = BigInteger.probablePrime(N/4, random);
      BigInteger r = BigInteger.probablePrime(N/4, random);
      BigInteger s = BigInteger.probablePrime(N/4, random);
      BigInteger phiN = (p.subtract(one)).multiply(q.subtract(one));
      BigInteger phiM = (r.subtract(one)).multiply(s.subtract(one));
      BigInteger phi = phiN.multiply(phiM);

      modulusN    = p.multiply(q);                                  
      modulusM    = r.multiply(s);                                  
      modulus     = modulusN.multiply(modulusM);
